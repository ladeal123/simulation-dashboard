...
with open('/workspace/data/股票池_长格式_final_clean.csv', encoding='utf-8-sig') as f:
...
with open('/workspace/data/股票池_长格式_含开盘价_qf.csv', encoding='utf-8-sig') as f:
    for row in csv.DictReader(f):
        try:
            o=float(row['open_qf'])
...
with open('/workspace/data/成交量.csv', encoding='utf-8') as f:
    for row in csv.DictReader(f):
        try:
            v=float(row['volume'])
...
# 指数 V4 状态机
idx_data={}
wb=openpyxl.load_workbook('/workspace/user_input_files/中证1000指数价格后复权.xlsx', data_only=True)
ws=wb.active
for row in range(5, ws.max_row+1):
    d=ws.cell(row=row,column=1).value; p=ws.cell(row,column=2).value
    if d is None: break
    d_str=d.strftime('%Y-%m-%d') if isinstance(d,datetime) else str(d)
    try: idx_data[d_str]=float(p)
    except: continue
wb.close()
...
def calc_vol_ratio(code, signal_date, date_idx, vol_data, dates_valid):
    """计算5日平滑量比, 返回avg_r或None"""
    vd=vol_data.get(code)
    if not vd or not signal_date:
        return None
    ratios=[]
    for off in range(5):
...
ytb_skip_count=0
sell_vol_skip_count=0  # 缩量死叉跳过次数
dump_sell_count=0      # 放量下跌卖出次数
...
print("\n"+"="*70)
print(" V46 v62_v20_final_v2 (量比加分+卖出端:缩量死叉<0.6跳过+放量下跌卖出)")
print("="*70)
...
print(f"缩量死叉跳过: {sell_vol_skip_count} | 放量下跌卖出: {dump_sell_count}")
print(f"\n卖出原因分布: {dict(sell_reason_counter)}")

result={
    'label':'V46 v62_v20_final_v2 (量比加分+卖出端:缩量死叉<0.6跳过+放量下跌卖出)',
    'final_nav':final_nav,
...
with open('/workspace/backtest_v46_v62_v20_final_v2.json','w') as f:
    json.dump(result, f, ensure_ascii=False, indent=2, default=str)
print(f"\n已保存到 /workspace/backtest_v46_v62_v20_final_v2.json")

with open('/workspace/V46_v62_v20_final_v2_trades.csv','w', newline='') as f:
    w=csv.DictWriter(f, fieldnames=['date','code','dir','price','shares','pnl','reason','yr','state'])
    w.writeheader()
    for t in trades_log: w.writerow(t)
print(f"交易记录: /workspace/V46_v62_v20_final_v2_trades.csv ({len(trades_log)}笔)")