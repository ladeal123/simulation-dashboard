...
# 方向2: DIF%短期动量减速 (最优: th=0.3, pen=0.20)
MOM_TH = 0.3
MOM_PEN = 0.20
# 方向3: 连续上涨天数惩罚 (最优: th=3, pen=0.02)
RISE_TH = 3
RISE_PEN = 0.02
POOL_TOP_N = 200
...
_wb_data = _xl.load_workbook('/workspace/股票池数据_new.xlsx', data_only=True, read_only=True)
_ws_data = _wb_data['Sheet1']
...
for code, dp in stock_data.items():
    ...
    # 方向2: DIF% 3日动量 (短期增速)
    dif_pct_mom3 = [None] * len(prices)
    for j in range(3, len(prices)):
        if dif_pct[j] is not None and dif_pct[j-3] is not None:
            dif_pct_mom3[j] = dif_pct[j] - dif_pct[j-3]
    IND[code]['dif_pct_mom3'] = dif_pct_mom3
    # 方向3: 连续上涨天数
    consec_rise = [0] * len(prices)
    for j in range(1, len(prices)):
        if prices[j] is not None and prices[j-1] is not None and prices[j] > 0 and prices[j-1] > 0:
            if prices[j] > prices[j-1]:
                consec_rise[j] = consec_rise[j-1] + 1
            else:
                consec_rise[j] = 0
    IND[code]['consec_rise'] = consec_rise
...
# ===== 加载开盘价(后复权) 用于信号生成 =====
print("加载开盘价(后复权v3乘法法)...")
import pandas as pd
open_df = pd.read_parquet('/workspace/股票池开盘价_后复权_v3.parquet')
print(f"  开盘价: {len(open_df)}天 x {len(open_df.columns)}只")
sys.stdout.flush()

def get_open(code, date):
    """获取后复权开盘价"""
    try:
        if date in open_df.index and code in open_df.columns:
            v = open_df.loc[date, code]
...
for di, date in enumerate(sim_dates):
    ...
    def _sort2_score(c):
        ...
        # 方向2: DIF%动量减速
        dif_mom = 0
        mom_arr = ind_c.get('dif_pct_mom3')
        if mom_arr is not None and _i_vol < len(mom_arr) and mom_arr[_i_vol] is not None:
            v_mom = mom_arr[_i_vol]
            if v_mom > MOM_TH:
                dif_mom = -MOM_PEN * (v_mom - MOM_TH)
        # 方向3: 连续上涨天数惩罚
        rise_pen = 0
        rise_arr = ind_c.get('consec_rise')
        if rise_arr is not None and _i_vol < len(rise_arr) and rise_arr[_i_vol] is not None:
            streak = rise_arr[_i_vol]
            if streak > RISE_TH:
                rise_pen = -RISE_PEN * (streak - RISE_TH)
        return s - VOL_ALPHA * v + DIF_BONUS * dif_pct + dif_mom + rise_pen
...
print(f"\n{'='*60}")
print(f"V48 Final D2+D3 (动量减速+连涨惩罚)")
print(f"最终净值: {final_nav:.4f}  ({(final_nav-1)*100:+.2f}%)")
print(f"期末现金: {cash:,.0f}")

wins = [t for t in trades if t['浮盈亏金额'] > 0]
losts = [t for t in trades if t['浮盈亏金额'] <= 0]
print(f"总交易: {len(trades)}, 胜率: {len(wins)/max(1,len(trades))*100:.1f}%")
print(f"盈利笔均: {sum(t['浮盈亏金额'] for t in wins)/max(1,len(wins)):+,.0f}元")
print(f"亏损笔均: {sum(t['浮盈亏金额'] for t in losts)/max(1,len(losts)):+,.0f}元")
...
out_file = '/workspace/V48_final_D2D3_result.json'
