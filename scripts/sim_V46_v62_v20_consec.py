"""模拟盘 V46_v62_v20_consec: T日收盘信号 + T+1开盘执行 + 完整评分
策略: MACD(10,20,9) 3日金叉选股
- 选股: BAR>0, 价格>MA144, 3日内金叉
- 排序: (DIF%x0.3 + BAR%x0.7) - 0.2xvol10
- 量比加分: 5日均量比 >=1.0/1.2/1.5 -> +10%/+20%/+35%
- 超跌反弹: 近10日最大回撤>8%/+15% -> +8%/+15%
- MA144斜率: 明显向上+10% 微向上+5% 向下-5%
- 连涨过滤: 连涨7天-10% 5天-5% 3天-2%
...
INDEX_FAST, INDEX_SLOW, INDEX_DEA = 10, 20, 9
...
def get_close(code, date):
    ...
    return dp.get(date) if dp else None
...
# ========== 主回测 ==========
print("[6/7] 开始回测 (T日收盘信号 + T+1开盘执行 + 完整评分)...", flush=True)
...
for di, date in enumerate(sim_dates):
    ...
    for code, ind in IND.items():
        ...
        rev_bonus = 0.0
        if i >= 10:
            look_prices = ind['prices'][i-9:i+1]
        ...
        if not math.isnan(ma144_i):
            if i >= 5:
                ma144_5_i = ind['ma144'][i-5]
                if not math.isnan(ma144_5_i) and ma144_5_i > 0:
                    slope = (ma144_i - ma144_5_i) / ma144_5_i
                    if slope > 0.005:
                        slope_bonus = 0.10
                    elif slope > 0.001:
                        slope_bonus = 0.05
        ...
        if i >= 5:
            ...
            for off in range(1, 11):
                if i - off < 0:
                    break
                p_today = ind['prices'][i-off+1]
                p_yest = ind['prices'][i-off]
        ...
        score = base_score * GC_PENALTY.get(gc_days, 0.2) * (1 + vol_bonus + rev_bonus + slope_bonus + consec_bonus)
        if vol_bonus > 0:
    ...
    if prev_day_rank:
        ...
        for idx, c in enumerate(candidates[:100]):
            ...
            if prev_rank is not None:
                ...
                if change > 0:
                    rank_changes.append({
                        '日期': signal_date, '代码': code,
                        '名称': code_to_name.get(code, ''),
                        '指数归属': get_index_tag(code),
                        '池内': is_in_pool(code),
                        '当日排名': cur_rank, '昨日排名': prev_rank,
                        '排名变化': change, '得分': round(c['score'], 4),
                        '量比': c['vol_ratio'],
                    })
...
# ========== 最终结算 ==========
print("[7/7] 最终结算 + 输出Excel...", flush=True)
...
print(f"\n{'=' * 60}")
print(f"V46_v62_v20_consec (量比+超跌+MA144斜率+连涨过滤 + T+1开盘)")
...
stats = [
    ...
    {'指标': '超跌反弹加分', '数值': rev_bonus_count},
    {'指标': 'MA144斜率正/负', '数值': f'{slope_bonus_pos}/{slope_bonus_neg}'},
    {'指标': '连涨过滤惩罚', '数值': consec_penalty_count},
]
...
print(f"  Sheet6 每日涨幅最快Top10: {len(daily_rank_up_sorted)} 行")
