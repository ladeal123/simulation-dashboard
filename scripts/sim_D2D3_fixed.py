...
for code, dp in stock_data.items():
    ...
    # D2: DIF% 3日动量
    mom3 = [None] * len(prices)
    for j in range(3, len(prices)):
        if dif_pct[j] is not None and dif_pct[j-3] is not None:
            mom3[j] = dif_pct[j] - dif_pct[j-3]
    IND[code]['dif_pct_mom3'] = mom3
    # D3: 连续上涨天数
    cr = [0] * len(prices)
    for j in range(1, len(prices)):
        if prices[j] and prices[j-1] and prices[j] > 0 and prices[j-1] > 0:
            cr[j] = cr[j-1] + 1 if prices[j] > prices[j-1] else 0
    IND[code]['consec_rise'] = cr
...
for di, date in enumerate(sim_dates):
    ...
    def _sort2_score(c):
        ...
        # D2: DIF%动量减速
        dm = 0; ma = ind_c.get('dif_pct_mom3')
        if ma is not None and _i_vol < len(ma) and ma[_i_vol] is not None:
            vm = ma[_i_vol]
            if vm > MOM_TH: dm = -MOM_PEN * (vm - MOM_TH)
        # D3: 连涨惩罚
        rp = 0; ra = ind_c.get('consec_rise')
        if ra is not None and _i_vol < len(ra) and ra[_i_vol] is not None:
            st = ra[_i_vol]
            if st > RISE_TH: rp = -RISE_PEN * (st - RISE_TH)
        return s - VOL_ALPHA * v + DIF_BONUS * dif_pct + dm + rp
...
print(f"\n{'='*60}")
print(f"模拟盘 D2+D3版 (回测引擎, {START_DATE}~{END_DATE})")
...
out_file = f'/workspace/模拟盘_D2D3版_固定_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}'
